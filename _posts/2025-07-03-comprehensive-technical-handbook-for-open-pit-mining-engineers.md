...
*   **Fleet Sizing (Simplified):** To determine the number of trucks required for a given number of shovels, the following simplified formula can be used:
    ...
    *   $N_{shovels}$ = Number of shovels
    *   $Truck\[Cycle\]\[Time\]$ = Time for a truck to complete one cycle (loading, hauling, dumping, returning)
    *   $Shovel\[Loading\]\[Time\]$ = Time for a shovel to load one truck
...
*   $Processing\[Cost\]$ = Cost per tonne of ore processed
*   $Mining\[Cost\]\[of\]\[Ore\]$ = Cost per tonne of ore mined
*   $Mining\[Cost\]\[of\]\[Waste\]$ = Cost per tonne of waste mined
